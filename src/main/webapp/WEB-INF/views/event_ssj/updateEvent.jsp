		...
		<form action="updateEvent.ev" method="post" enctype="Multipart/form-data" onsubmit="return submitForm();">
			...
			<table class="tableStyle">
				<tr>
					...
					<td>
						<input type="text" class="inputStyle" name="eTitle" id="event_title" placeholder="이벤트 제목을 입력하세요." value="${ event.eTitle }" required>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<select size="1" id="event_entry" name="eTarget">
							<option value="모든 회원" <c:if test="${ event.eTarget == '모든 회원'}">selected</c:if>>모든 회원</option>
							<option value="후원자" <c:if test="${ event.eTarget == '후원자'}">selected</c:if>>후원자</option>
							<option value="오디션 지원자" <c:if test="${ event.eTarget == '오디션 지원자'}">selected</c:if>>오디션 지원자</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="number" min="1" id="event_num" name="eEventNum" value="${ event.eEventNum }" required>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="2"><textarea class="textClass" name="eContent" id="event_intro" rows="7" cols="110" placeholder="이벤트 소개를 적어주세요." required>${ event.eContent }</textarea></td>
				</tr>
				<tr>
					<td colspan="2" class="label" id="labelTd1">이벤트 참여 방법</td>
				</tr>
				<tr>
					<td colspan="2"><textarea class="textClass" name="eWay" id="event_way" rows="7" cols="110" placeholder="이벤트 참여 방법을 적어주세요." required>${ event.eWay }</textarea></td>
				</tr>
				<tr>
					<td colspan="2" class="label" id="labelTd1">이벤트 당첨 기준</td>
				</tr>
				<tr>
					<td colspan="2"><textarea class="textClass" name="eStand" id="event_stand" rows="7" cols="110" placeholder="이벤트 당첨 기준을 적어주세요." required>${ event.eStand }</textarea></td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="2" class="btnStyle">
						<input type="submit" class="defaultBtn upBtn" value="수정">
						<button class="defaultBtn delBtn" onclick="location.href='javascript:history.go(-1);'">취소</button>
					</td>
				</tr>
			</table>
		</form>
